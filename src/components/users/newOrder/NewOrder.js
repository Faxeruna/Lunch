...
export default class HomePage extends Component {
  ...
  handleDelete = item => {
    const array = this.state.new_order;
    const itemFound = array.indexOf(item);
    if (itemFound > -1) {
      array.splice(itemFound, 1);
    }
    this.setState({ new_order: array });
  };
  ...
  addItem(product, limit_catalog) {
    if (this.state.count[product.id_product]) {
      ...
      if (temp_order[id_product] && temp_order[id_product].count) {
        var countProduct =
          Number(temp_order[id_product].count) +
          Number(this.state.count[product.id_product]);
      } else {
        var countProduct = this.state.count[product.id_product];
      }
      if (countProduct >= 0) {
        if (!limit_catalog) {
          temp_order[id_product] = {
            name: name,
            count: countProduct,
          ...
          if (countProduct > limit_catalog) {
            alert(
              "Вы превысили лимит. Введите количество в соответствии с лимитом."
            );
          } else {
            temp_order[id_product] = {
              name: name,
  ...
  createOrder(e) {
    if (this.state.new_order) {
      axios({
        method: "post",
        url: "https://simbirsoftlearning.herokuapp.com/api_lunch_system.php?mode=create_order",
        data: {
  ...
  componentDidMount() {
    axios({
      method: "post",
      url: "https://simbirsoftlearning.herokuapp.com/api_lunch_system.php?mode=get_catalog",
      data: {
        session_token: USER[0].session_token
      }
    })
                  ...
                  <Card bg="link" style={{ width: "22rem" }} key={index}>
                    <Card.Header>
                      <Accordion.Toggle
                        ...
                        eventKey={index}
                      >
                        <b>{item_catalog.name}</b> Общий лимит:{" "}
                        {item_catalog.count ? item_catalog.count : "Неогр."}
                      </Accordion.Toggle>
                    </Card.Header>
                  ...
                  <Row className="border my-2 px-0">
                    <Card bg="success" text="dark" style={{ width: "22rem" }}>
                      ...
                      <Card.Body>
                        ...
                        <Card.Title>
                          <b>Пользователь:</b> {USER[0].name}
                        </Card.Title>
                        <Card.Title>
                          <b>Город: </b> {USER[0].city}
                        </Card.Title>
                        <Card.Title>
                          <b>Кабинет:</b> {USER[0].location}
                        </Card.Title>
                        <Card.Title>
                          <b>Дата: </b> {date}
                        </Card.Title>
